...
export function CredentialItem({
  ...
  const activeTypes: Set<string> = new Set(linkedModels.map(m => m.type))
  ...
  if (defaults) {
    const slotMap: Record<string, string | null | undefined> = {
      ...
      'Reranker': defaults.default_reranker_model,
    }
      ...
      <div className="border rounded-lg p-3 space-y-2">
        ...
        {linkedModels.length > 0 && (
          <div className="space-y-1.5 pt-1">
            {(['language', 'embedding', 'reranking', 'image_generation', 'audio', 'video', 'text_to_speech', 'speech_to_text'] as ModelType[])
