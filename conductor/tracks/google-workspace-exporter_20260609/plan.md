...
### Tasks
- [x] **Task 3.1**: Refactor `compile_markdown_to_docx` in `api/routers/notebooks.py` to compile styles (fonts, colors, margins) dynamically based on active `StyleGuide` database models.
- [x] **Task 3.2**: Implement `POST /notebooks/export/gdocs` utilizing `googleapiclient.discovery` to generate formatted Docs.
- [x] **Task 3.3**: Implement `POST /notebooks/export/gslides` converting canvas drawings into slides shapes.
- [x] **Task 3.4**: Implement `POST /notebooks/export/gsheets` generating structured spreadsheets for compliance scoring checklists.

### Verification
- [x] Run integration tests `tests/test_google_workspace_exporter.py` validating that document compilation, node conversion, and sheets checklist generation complete successfully.
