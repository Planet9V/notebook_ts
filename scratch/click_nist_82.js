...
async function main() {
  ...
  const goBackResult = await send('Runtime.evaluate', {
    expression: `(() => {
      const returnBtn = Array.from(document.querySelectorAll('button'))
        .find(b => b.textContent.trim().includes('Return to CSET') || b.textContent.trim().includes('CSET Regulatory'));
      if (returnBtn) {
        returnBtn.click();
  ...
  const clickResult = await send('Runtime.evaluate', {
    expression: `(() => {
      // Find all elements that contain the framework name
      const elements = Array.from(document.querySelectorAll('*'));
      const targetElement = elements.find(el => {
        const childNodes = el.childNodes;
        return childNodes.length === 1 && childNodes[0].nodeType === 3 && childNodes[0].nodeValue.trim() === 'NIST SP 800-82 r3';
      });
      ...
      const nistCard = cards.find(c => c.innerText.includes('NIST SP 800-82 r3'));
      if (nistCard) {
        nistCard.click();
        return 'Clicked NIST SP 800-82 r3 card via innerText fallback';
      }

      return 'Could not find NIST SP 800-82 r3 card anywhere on the page';
    })()`,
    returnByValue: true
  });
